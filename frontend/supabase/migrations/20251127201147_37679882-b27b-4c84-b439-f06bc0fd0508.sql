-- Remove the problematic triggers that rely on runtime configuration
-- We'll handle email sending via edge function calls from the application instead

DROP TRIGGER IF EXISTS on_release_status_change ON public.releases;
DROP FUNCTION IF EXISTS public.send_release_status_email();

-- Simplify the handle_new_user function to remove the email sending logic
-- Email sending will be handled by the application after signup
